...
public class c5_CreatingSequence {
    ...
    public void repeat() {
        ...
        Callable<Integer> incrementCounterCallable =  () -> counter.incrementAndGet();
        Flux<Integer> repeated = Mono
            .fromCallable(incrementCounterCallable)
            .repeat(9);
     ...
     * The difference between `generate` and `create` is that 
     * `generate` let us emit only once per callback call
     * while `create` let us emit multiple times per round.
     * - What is difference between `create` and `push`?
     * The difference between `create` and `push` is that while
     * `create` may create several producing threads,
     * `push` can invoke only one producing thread at a time.
     */
    @Test
